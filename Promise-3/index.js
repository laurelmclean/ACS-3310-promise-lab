...
// What happens? 
greet(234) // Returns a Promise
  .then(str => uppercaser(str))  
  .then(str => console.log(str)) 
  .catch(err => console.log(err)) // Catches an error - returns greet expects a string

// Challenge: get uppercaser() to fail by passing a non string value
// What happens? 
uppercaser(234) // Returns a Promise
  .then(str => greet(str))
  .then(str => console.log(str))
  .catch(err => console.log(err)) // Argument to uppercaser must be string

// Challenge: Notice there is only a single .catch() at the end. 
// Explain the behavior?

// if any promise in a chained promise rejects, all of the promises reject and you skip to the single catch callback at the end
